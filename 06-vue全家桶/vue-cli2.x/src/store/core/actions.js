export default {
  ...
  // 也可以通过 payload 传递参数进来，payload 可以是几个简单参数，也可以是一个对象参数，页面通过 $store.dispatch('incrementAsync', payload)
  ...
  incrementAsync(context, payload) {
    ...
    //   // 这里可以放一些 axios 请求去请求后台数据
    //   // resolve() 之后。在 vue 文件中，使用 $store.dispatch('incrementAsync').then(() => { // 这里可以获取到修改后的 state 的数据 })
    //   setTimeout(() => {
    //     context.commit('increment');
    //     resolve('异步处理完成');
    //   }, 1000);
